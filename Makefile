...
gotools:
	mkdir -p build/bin
	cd gotools && $(MAKE) install BINDIR=$(CURDIR)/build/bin
...
build/docker/bin/%: $(PROJECT_FILES)
	...
	@docker run -i \
		...
		hyperledger/fabric-baseimage:$(BASE_DOCKER_TAG) go install -ldflags "$(GO_LDFLAGS)" $(PKGNAME)/$(TARGET)
	@touch $@
...
build/image/membersrvc/config: membersrvc/membersrvc.yaml
	mkdir -p $@
	cp $< $@
